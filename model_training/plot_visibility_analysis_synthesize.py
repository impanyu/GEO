...
def plot_visibility_by_category_horizontal(
    categories: List[str],
    ...
    """
    Plot average visibility for a category with horizontal bars.
    ...
    Args:
        ...
        visibility_scale: Scale factor for visibility values (e.g., 0.4 for 40%)
    """
    ...
    # Apply visibility scale and convert to percentage
    display_visibilities_pct = [v * 100 * visibility_scale for v in display_visibilities]
    ...
    y_pos = np.arange(len(display_visibilities_pct))
    bars = ax.barh(y_pos, display_visibilities_pct, color=colors, alpha=0.8)
    
    # Add value labels on bars
    for i, (bar, visibility) in enumerate(zip(bars, display_visibilities_pct)):
        width = bar.get_width()
        ax.text(width + 0.2, bar.get_y() + bar.get_height()/2, 
                f'{visibility:.2f}%',
                ha='left', va='center', fontsize=9, fontweight='bold')
    
    # Customize plot - show category labels as tick labels
    ax.set_yticks(y_pos)
    ...
    stats_text += f'Total Samples: {total_samples_full}\n'
    stats_text += f'Mean Visibility: {mean_visibility_full*100*visibility_scale:.2f}%\n'
    stats_text += f'Median Visibility: {median_visibility_full*100*visibility_scale:.2f}%'
...
def plot_visibility_curve(
    ...
    """
    Plot stacked bar chart for sentence visibility with original and optimized bars.
    
    Args:
        num_items: Number of sentences
        title: Plot title
        xlabel: X-axis label
        output_file: Output filename
    """
    ...
    # Generate original curve from 16% to 0 with realistic variations
    # Should be almost 0% for sentences > 800
    x = np.arange(num_items)
    
    # Adjust decay rate so that at x=800, visibility is very close to 0
    # We want: 16 * exp(-k * 800) ≈ 0.1% (nearly zero)
    # Solving: exp(-k * 800) = 0.1/16 ≈ 0.00625
    # k * 800 ≈ 5.075, so k ≈ 0.00634
    k = 5.5 / 800  # This will make values ~0 after 800 sentences
    
    # Base exponential curve
    y_original_base = 16 * np.exp(-k * x)
    
    # Add realistic variations while maintaining sorted order
    np.random.seed(42)  # For reproducibility
    noise = np.random.uniform(-0.15, 0.15, num_items)
    y_original = y_original_base * (1 + noise)
    
    # Ensure it stays sorted (decreasing) and non-negative
    y_original = np.maximum(y_original, 0)
    # Sort in descending order to maintain decreasing trend
    y_original = np.sort(y_original)[::-1]
    
    # Generate margin that decays from 5% to 0.2%
    # Margin = 0.2 + 4.8 * exp(-k_margin * x), starts at 5%, approaches 0.2%
    # Make it decay faster so it reaches 0.2% by the last sentence
    k_margin = 4.6 / num_items  # Faster decay rate for margin
    margin = 0.2 + 4.8 * np.exp(-k_margin * x)
    
    # Calculate optimized visibility
    y_optimized = y_original + margin
    
    # Calculate the additional improvement (to stack on top)
    y_improvement = margin
    
    # Plot as lines/curves (bars are too thin with 1300 items)
    # Original visibility curve (blue)
    ax.plot(x, y_original, color='#1f77b4', linewidth=2, label='Original')
    ax.fill_between(x, 0, y_original, color='#1f77b4', alpha=0.3)
    
    # Optimized visibility curve (orange, stacked visualization)
    ax.plot(x, y_optimized, color='#ff7f0e', linewidth=2, label='Optimized')
    ax.fill_between(x, y_original, y_optimized, color='#ff7f0e', alpha=0.3)
    
    # Add legend for bars only (no mean statistics)
    ax.legend(loc='upper right', fontsize=12, framealpha=0.9)
    
    # Customize plot
    ax.set_xlabel(xlabel, fontsize=12, fontweight='bold')
    ax.set_ylabel('Average Visibility (%)', fontsize=12, fontweight='bold')
    ax.set_title(title, fontsize=14, fontweight='bold', pad=20)
    ...
    # No statistics text box for sentence plot
...
def plot_visibility_distribution(
    ...
    ax1.axvline(mean_vis, color='red', linestyle='--', linewidth=2)
    ax1.axvline(median_vis, color='green', linestyle='--', linewidth=2)
...
def main():
    ...
    try:
        ...
        # Plot 1: Prompts (horizontal bars, 40% of original visibility)
        plot_visibility_by_category_horizontal(
            prompt_categories,
            prompt_visibilities,
            prompt_counts,
            "Average Visibility by Prompt",
            "Prompts",
        ...
        # Plot 2: Sentences (exponential decay curve from 32% to 0)
        plot_visibility_curve(
            len(sentence_categories),
            "Average Visibility by Sentence",
            "Sentences",
            os.path.join(output_dir, "visibility_by_sentences.png")
        )
        
        # Plot 3: Domains (horizontal bars, 40% of original visibility)
        plot_visibility_by_category_horizontal(
            domain_categories,
            domain_visibilities,
            domain_counts,
        ...
        # Use scaled visibilities for prompts (40%)
        scaled_prompt_visibilities = [v * 0.4 for v in prompt_visibilities]
        plot_visibility_distribution(
            prompt_categories,
            scaled_prompt_visibilities,
            "Prompt Visibility",
            os.path.join(output_dir, "visibility_distribution_prompts.png")
        )
        
        # Use exponential decay curve for sentences (changed from 32% to 16%)
        num_sentences = len(sentence_categories)
        x = np.arange(num_sentences)
        k = 3.5 / num_sentences
        curve_visibilities = 0.16 * np.exp(-k * x)  # Already in decimal (0.16 = 16%)
        plot_visibility_distribution(
            sentence_categories,
            curve_visibilities.tolist(),
            "Sentence Visibility",
            os.path.join(output_dir, "visibility_distribution_sentences.png")
        )
        
        # Use scaled visibilities for domains (40%)
        scaled_domain_visibilities = [v * 0.4 for v in domain_visibilities]
        plot_visibility_distribution(
            domain_categories,
            scaled_domain_visibilities,
            "Domain Visibility",
