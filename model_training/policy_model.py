...
class GRPOTrainer:
    ...
    async def generate_and_score(self, batch) -> Tuple[List[torch.Tensor], List[float], List[float]]:
        ...
        for i in range(len(batch['input_ids'])):
            ...
            for sample_idx in range(self.config.num_samples_per_input):
                ...
                generated_text = self.tokenizer.decode(generated_sequence[input_length:], skip_special_tokens=True)
    ...
    async def calculate_rewards_tmp(self, batch, generated_sentences: List[List[str]]) -> List[float]:
        """
        Calculate rewards using two-component system: quality assessment + length measurement
        Same implementation as predict_visibility_tmp in inference.py for consistency
        ...
        for i in range(len(generated_sentences)):
            try:
                ...
                logger.debug(f"Sample {i}: Brand={brand_name}, Quality={quality_score:.3f}, Length={length_score:.3f}, Final={final_score:.3f}")
...
"""
            ...
            if response.status_code != 200:
                logger.error(f"OpenRouter API error during suggestion cleaning: {response.status_code} - {response.text}")
                return raw_generated_text.strip()  # Fallback to raw text
            ...
            logger.debug(f"Successfully cleaned suggestions using GPT-4o")
            return cleaned_suggestions
                
        except Exception as e:
            logger.error(f"Error cleaning suggestions with GPT-4o: {e}")
            return raw_generated_text.strip()  # Fallback to raw text
    ...
    async def _extract_and_apply_suggestions_with_gpt4o(
        ...
        try:
            ...
            suggestions = await self._clean_suggestions_with_gpt4o(
                generated_text, original_sentences, brand_name, dimension, domain
            )
